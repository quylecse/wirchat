...
export const signUp = async (req, res) => {
    try {
        ...
        if (!username || !password || !email || !firstName || !lastName) {
            return res.status(400).json({ message: "Bitte überprüfen Sie Ihre Eingaben" })
        }
        ...
        if (duplicateUser) {
            if (duplicateUser.username === username) {
                return res.status(409).json({ message: "Benutzername ist bereit vorhanden!" });
            }
            if (duplicateUser.email === email) {
                return res.status(409).json({ message: "E-Mail ist bereit vorhanden!" });
            }
        }
...
export const signIn = async (req, res) => {
    try {
        ...
        if (!username || !password) {
            return res.status(400).json({ message: "E-Mail-Adresse oder Telefonnummer eingeben" });
        }

        //username auf DB nachschlagen
        const user = await User.findOne({ username });
        if (!user) {
            return res.status(401).json({ message: "Ihr Konto wurde nicht gefunden." });
        }
